...
static inline Eigen::Matrix<amplitude_t, Eigen::Dynamic, Eigen::Dynamic> perform_filling (const std::vector<lw_vector<int, MAX_DIMENSION> > &momentum_sites, const Lattice &lattice, const BoundaryConditions &bcs)
{
    ...
    for (unsigned int i = 0; i < N_filled; ++i) {
        ...
        lw_vector<boost::rational<int>, MAX_DIMENSION> momentum = allowed_momentum(momentum_sites[i], lattice, bcs);
...
FilledOrbitals::FilledOrbitals (const std::vector<lw_vector<int, MAX_DIMENSION> > &momentum_sites, const boost::shared_ptr<const Lattice> &lattice_, const BoundaryConditions &bcs)
