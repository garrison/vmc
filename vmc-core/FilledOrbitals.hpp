...
public:
    ...
    static Eigen::Matrix<amplitude_t, Eigen::Dynamic, Eigen::Dynamic> perform_filling (const std::vector<boost::array<int, DIM> > &momentum_sites, const NDLattice<DIM> &lattice, const typename NDLattice<DIM>::BoundaryConditions &bcs)
        {
            ...
            for (unsigned int i = 0; i < N_filled; ++i) {
                ...
                boost::array<boost::rational<int>, DIM> momentum = allowed_momentum(momentum_sites[i], lattice, bcs);
                ...
                for (unsigned int j = 0; j < N_sites; ++j) {
                    const typename NDLattice<DIM>::Site site = lattice.site_from_index(j);
