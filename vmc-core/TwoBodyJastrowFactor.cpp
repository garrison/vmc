    ...
    : m_correlation_matrix(correlation_matrix)
{
    ...
    assert(correlation_matrix.rows() == correlation_matrix.cols());
    assert(correlation_matrix == correlation_matrix.transpose());
}

template<typename AmplitudeType>
Big<AmplitudeType> TwoBodyJastrowFactor<AmplitudeType>::compute_jastrow (const PositionArguments &r) const
{
    assert(r.get_N_sites() == m_correlation_matrix.cols());
