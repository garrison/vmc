...
void RVBWavefunctionAmplitude::move_particle_ (Particle particle, unsigned int new_site_index)
{
    ...
    const int other_particle_ind = r.particle_index_at_pos(new_site_index, other_species);
    BOOST_ASSERT(other_particle_ind != -1);
    Particle other_particle(other_particle_ind, other_species);
...
void RVBWavefunctionAmplitude::reset_with_filler (const RandomFiller &filler, rng_class &rng)
{
    ...
    const unsigned int Nsites = lattice->total_sites();
    BOOST_ASSERT(Nsites == r.get_N_sites());
    
    BOOST_ASSERT(r.get_N_filled_total() == Nsites);  // Assert a spin wave function!
    ...
    for (unsigned int i = 0; i < Nsites; ++i) {
        ...
        if (it == vv[0].end()) vv[1].push_back(i);
    }
    
    BOOST_ASSERT(vv[0].size() == vv[1].size());
    reset(PositionArguments(vv, Nsites));
}